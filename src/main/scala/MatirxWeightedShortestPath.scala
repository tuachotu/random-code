...
object MatirxWeightedShortestPath extends App {
        ...
        if (pos._1 == destination) {
          ...
          possibleMoves(matrix, pos._1) foreach { move =>
            if (!visited.contains(move)) {
              ...
              lookupTable.enqueue((move, pos._2 + matrix(move.row)(move.col)))
            }
          }
        }
      }
  ...
  println(distance(Location(0,0), Location(2,2)))
}
